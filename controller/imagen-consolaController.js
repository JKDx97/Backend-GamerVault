...
exports.uploadImage = async (req, res) => {
  try {
    const { consola_id } = req.body;

    if (!consola_id || !req.file) {
    ...
    db.query(
      "INSERT INTO images_consolas (consola_id, consola_image_principal, consola_image_url) VALUES (?, ?, ?)",
      [consola_id, imageUrl, "NO"],
      (err, result) => {
...
exports.getImagesByConsolaId = (req, res) => {
  db.query(
    "SELECT * FROM images_consolas WHERE consola_id = ?",
    [req.params.consola_id],
    (err, results) => {
      if (err) return res.status(500).json({ error: err.message });
      res.json(results);
    }
...
exports.deleteImage = async (req, res) => {
  ...
  db.query(
    "SELECT consola_image_url FROM images_consolas WHERE id = ?",
    [id],
    async (err, results) => {
